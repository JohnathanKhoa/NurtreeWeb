      ...
      <div >
        <Menu>
          <MenuButton as={Fragment}>
            ...
            }} className={` ${clsx(active)}`}><div className='flex flex-row p-4 gap-2 items-center  justify-center '><Plus size={25}/>Add to Playlist</div></button>}
          </MenuButton>
          <MenuItems anchor="bottom" className="overflow-auto scrollbar-hide bg-zinc-800 rounded-xl opacity-80">
            <div onClick={() =>{handleClick(user, trackId)}} className=" rounded-md text-white cursor-pointer  hover:bg-zinc-500"><div className='flex flex-row p-4 gap-2 items-center  '><Plus size={25}/>New Playlist</div></div>
