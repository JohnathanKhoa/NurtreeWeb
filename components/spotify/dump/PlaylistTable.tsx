...
export default function PlaylistTable({
  ...
  function handleClick(playlist: Playlist, index: number) {
      ...
      <div className={`${tableIsOpen ? "visible" : "hidden"}`}> */}
        <>
          {showHeader && (
            <>
              <div className="sticky w-full overflow-hidden">
                <header className="sticky top-0 grid grid-cols-12 gap-2 p-4 pb-1 text-gray text-2xs">
          ...
          {/* Table Rows */}
          <div className="w-full col-span-12 mt-2 overflow-y-scroll h-100">
            {playlists?.map((playlist, index) => (
              <div key={keyCount++}>
                <div
                  onClick={() => {
                    handleClick(playlist, index);
                  }}
                  ...
                  key={playlist.id + index + keyCount++}
